	...
	: QDialog(parent), m_pixPath("")
{
	...
	ui.tableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);    //表格只读触发
...
void QtQQ_Server::updateTableData(int depID, int employeeID)
{
	...
	if (depID && depID != m_comDepID)    //查询部门
	{
	...
	//设置表头
	QStringList headers;
	headers << QStringLiteral("部门") << QStringLiteral("工号") << QStringLiteral("员工姓名")
		<< QStringLiteral("员工签名") << QStringLiteral("员工状态") << QStringLiteral("员工头像")
		<< QStringLiteral("在线状态");
...
void QtQQ_Server::initComboBoxData()
{
	...
	int depCounts = queryDepModel.rowCount() - 1;     //部门总数：减去一个公司群
	...
	for (int i = 0; i < depCounts + 1; i++)    //多一个公司群
	{
...
void QtQQ_Server::on_queryIDBtn_clicked()
{
	...
	{
		QMessageBox::warning(NULL, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("请输入员工QQ号！"));
		ui.queryIDLineEdit->setFocus();    //设置焦点
		return;
	}

	int employeeID = ui.queryIDLineEdit->text().toInt();   //获取用户输入的员工QQ号
	...
	if (!queryInfo.next())
	{
		QMessageBox::warning(NULL, QString::fromLocal8Bit("提示"), QString::fromLocal8Bit("请输入正确的员工QQ号！"));
		ui.queryIDLineEdit->setFocus();    //设置焦点
		return;  
	}
	else
...
void QtQQ_Server::on_logoutBtn_clicked()
{
	...
	int employeeID = ui.logoutIDLineEdit->text().toInt();   //获取用户输入的员工QQ号
...
void QtQQ_Server::on_addBtn_clicked()
{
	...
	int employeeID = maxEmployeeID.value(0).toInt() + 1;    //新的员工的QQ号
